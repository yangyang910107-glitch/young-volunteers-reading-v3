...
];STAGES.push({id:"awards",title:"MATCHING SUPERPOWERS"});const SENTENCES=PROFILES.flatMap(p=>p.sentences.map((text,i)=>({id:p.id+(i+1),who:p.id,text})));const EVIDENCE_UNITS=SENTENCES.flatMap(s=>(s.text.match(/[^,，.!?]+(?:[,，.!?]+|$)/g)||[s.text]).map((text,i)=>({id:s.id+'_'+i,parent:s.id,who:s.who,text:text.trim()})));if(typeof module!=='undefined')module.exports={STAGES,PROFILES,SENTENCES,EVIDENCE_UNITS,DEMO_SOURCE,MODEL_OPTIONS};else Object.assign(root,{STAGES,PROFILES,SENTENCES,EVIDENCE_UNITS,DEMO_SOURCE,MODEL_OPTIONS});})(typeof window!=='undefined'?window:globalThis);
