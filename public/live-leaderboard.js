...
function updateLiveLeaderboard(s){liveCurrent=s;if(!s.classStarted||!s.scoreboard||!s.revealed||!scoreTitles[s.stage])return;
...
function openLiveLeaderboard(s,animate=false,previous=null){if(!s)return;liveClear();if(!liveBoard){liveBoard=el('section',undefined,'live-leaderboard-overlay');liveBoard.setAttribute('role','dialog');liveBoard.setAttribute('aria-modal','true');liveBoard.setAttribute('aria-label','Group leaderboard');document.body.append(liveBoard);}
 ...
 const panel=el('div',undefined,'live-leaderboard-panel'),head=el('div',undefined,'live-leaderboard-heading'),title=el('h2','GROUP LEADERBOARD'),sub=el('p',scoreTitles[s.stage]||'LIVE CLASS'),close=el('button','CONTINUE →','live-board-continue'),board=el('div',undefined,'live-score-board');
 ...
 close.onclick=()=>{liveClear();liveBoard.hidden=true;document.querySelector('.teacher .actions button')?.focus();};close.focus();
...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&liveBoard&&!liveBoard.hidden){liveClear();liveBoard.hidden=true;}});
